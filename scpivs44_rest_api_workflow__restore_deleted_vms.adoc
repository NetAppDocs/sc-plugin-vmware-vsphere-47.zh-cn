...
{backupId}/restore` | `Restore VM from a backup using uuid or restore a deleted VM` 还原指定的备份。传递步骤 1 中的 * uuid* 。传递步骤 2 中的 * 备份 Id* 。传递 * restoreLocations* 属性中步骤 3 和 4 中的信息。如果VM备份是部分备份、请设置 `restartVM` 参数设置为"false"。* 注意： * 您无法还原模板虚拟机。 
|===